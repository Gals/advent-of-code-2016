...
class MyGivesLowToAndHighToInstruction(GivesLowToAndHighToInstruction):
	def add_chip_to_bot(self, bots, name, value):
		super(MyGivesLowToAndHighToInstruction, self).add_chip_to_bot(
			bots, name, value)
		expected_chips = [17, 61]
		if bots[name].chips == expected_chips:
			raise Exception('Bot %r is responsbile for comparing: %r chips' % (
				name, expected_chips))

INSTRUCTION_SET = [
	ValueGoesToInstruction,
	MyGivesLowToAndHighToInstruction,
]
